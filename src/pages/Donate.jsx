...
import { Heart, Building2, Utensils, Sprout, ShieldCheck, QrCode, CheckCircle2 } from 'lucide-react';
    ...
    return (
      <div className="container" style={{ padding: '8rem 0', textAlign: 'center' }}>
        ...
        <p className="text-light" style={{ maxWidth: '600px', margin: '0 auto 2rem', fontSize: '1.1rem' }}>
          Thank you for your contribution. Your transaction is now **Pending Verification**. 
          Admin will verify the UTR and screenshot within 24-48 hours.
        </p>
          ...
          <div className="content-card">
            {step === 1 ? (
              <form onSubmit={handleNext}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                    {causes.map(c => (
                      <div 
                        ...
                        style={{
                          padding: '0.75rem',
                          border: `2px solid ${form.category === c.id ? 'var(--color-primary)' : 'var(--border-color)'}`,
                          borderRadius: 'var(--radius-sm)',
                          cursor: 'pointer',
                          display: 'flex',
                          alignItems: 'center',
                          gap: '0.75rem',
                          background: form.category === c.id ? 'var(--color-primary-alpha)' : 'transparent'
                        }}
                      >
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 500 }}>Donation Amount (₹)</label>
                    ...
                    {amounts.map(amt => (
                      <button 
                        ...
                        style={{
                          ...
                          padding: '0.6rem',
                          borderRadius: 'var(--radius-sm)',
                          border: `2px solid ${form.amount === amt ? 'var(--color-primary)' : 'var(--border-color)'}`,
                          background: form.amount === amt ? 'var(--color-primary)' : 'transparent',
                          color: form.amount === amt ? 'white' : 'inherit',
                          fontWeight: 600
                        }}
                      >
                        {amt}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '2rem' }}>
                  ...
                  <div>
                    ...
                    <input type="tel" required style={inputStyle} value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} />
                  </div>
                </div>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <h3 style={{ marginBottom: '1rem', textAlign: 'center' }}>Step 2: Scan QR & Pay</h3>
                
                <div style={{ textAlign: 'center', marginBottom: '2rem', background: '#f9f9f9', padding: '1.5rem', borderRadius: 'var(--radius-md)' }}>
                  <div style={{ background: 'white', padding: '0.5rem', display: 'inline-block', borderRadius: 'var(--radius-sm)', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', marginBottom: '1rem' }}>
                    <img 
                      src="/src/assets/donation_qr.png" 
                      alt="Payment QR" 
                      style={{ width: '250px', display: 'block', borderRadius: '4px' }} 
                    />
                  </div>
                  <p style={{ fontWeight: 700, fontSize: '1.2rem', color: 'var(--color-primary)' }}>Payable Amount: ₹ {form.amount}</p>
                  <p style={{ fontWeight: 600, fontSize: '0.9rem', marginTop: '0.25rem' }}>UPI ID: 9792939973@paytm</p>
                  <p style={{ fontSize: '0.75rem', marginTop: '0.5rem', color: '#666' }}>Scan with any UPI App (PhonePe, GPay, Paytm)</p>
                </div>

                <div style={{ marginBottom: '1.5rem' }}>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 600 }}>Payment Transaction ID (UTR)</label>
                  <input 
                    type="text" 
                    required 
                    minlength="10"
                    placeholder="Enter 12-digit UTR number" 
                    style={{...inputStyle, border: '2px solid var(--color-primary)'}}
                    value={form.utr}
                    onChange={e => setForm({...form, utr: e.target.value})}
                  />
                  <p style={{ fontSize: '0.8rem', color: 'var(--color-text-light)', marginTop: '0.5rem' }}>
                    * Found in your UPI app payment summary.
                  </p>
                </div>

                <div style={{ marginBottom: '2rem' }}>
                  <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 600 }}>Upload Screenshot (Optional)</label>
